...
def inject_persistent_music(is_intro=False):
    ...
    Usa p._ytMusicPhase para controlar a transição:
      - 'intro': toca introVid (Quem Quer Ser Milionário) durante o GO! countdown
      - 'quiz':  toca quizVid em loop durante o quiz
    Não há vídeo de transição - vai direto de intro para quiz.
    """
    ...
    script = (
        ...
        # Se já está no quiz, não fazer nada
        "if(p._ytMusicPhase==='quiz')return;"
        # Se está em intro e agora queremos quiz, trocar música
        "if(p._ytMusicPhase==='intro'&&!isIntro){"
        "  if(p._ytPlayer&&p._ytPlayer.loadVideoById){"
        "    p._ytMusicPhase='quiz';"
        "    p._ytPlayer.loadVideoById({videoId:quizVid,startSeconds:0});"
        "    p._ytPlayer.setLoop(true);"
        "  }"
        "  return;"
        "}"
        # Se já está em intro e ainda é intro, não fazer nada
        "if(p._ytMusicPhase==='intro'&&isIntro)return;"
        # Criar player pela primeira vez
        "p._ytMusicPhase=isIntro?'intro':'quiz';"
        "var startVid=isIntro?introVid:quizVid;"
        "if(p.document.getElementById('_yt_persist'))return;"
        ...
        "    width:'1',height:'1',videoId:startVid,playerVars:pv,"
        "    events:{"
        "      onReady:function(e){e.target.setVolume(70);},"
        "      onStateChange:function(e){"
        "        if(e.data===0&&p._ytMusicPhase==='quiz'){p._ytPlayer.playVideo();}"
        "      }"
        "    }"
        "  });"
        "}"
